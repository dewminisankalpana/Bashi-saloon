                ...
                data.forEach(item => {
                    if (item.Quantity < 5) {
                        Swal.fire({
                            ...
                            cancelButtonText: 'Email Supplier Manager',
                        }).then((result) => {
                            if (result.isDismissed) {
                                const emailSubject = encodeURIComponent('Low Inventory Alert');
                                const emailBody = encodeURIComponent(`Dear Supplier Manager,\n\nThe inventory item no :"${item.ItemNo}" item name :"${item.ItemName}" is running low with only ${item.Quantity} units remaining.\n\nBest regards,\n Salon Bashi`);
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button className="relative inline-flex items-center justify-center p-0.5 me-2 overflow-hidden text-sm font-medium text-gray-100 rounded-lg group bg-gradient-to-br from-pink-900 to-pink-500 group-hover:to-pink-500 hover:text-white dark:text-black focus:ring-4 focus:outline-none focus:ring-purple-200 dark:focus:ring-purple-800">
                            <span className="relative px-5 py-2.5 transition-all ease-in duration-75 bg-pink dark:bg-gray-100 rounded-md group-hover:bg-opacity-0" onClick={() => (window.location.href = "/inventories/create")}>
                                Add
                            </span>
                        </button>
                    </div>
